...
#heading(level: 3, outlined: false)[Computational Facilities and Storage Strategy]

Due to the multi-dimensional hydrodynamical nature of Type Ia supernova simulations, access to High-Performance Computing (HPC) infrastructure is critical. Calculations were executed on *Genoa*, one of HITS’s dedicated HPC clusters running under the SLURM workload manager. *Genoa* is a CPU cluster composed of . Contrary to other GPUS or CPU clusters available at HITS (like *Bergamo*, *Roma* or *Cascade*), it is not located at the HITS facility but is hosted in a building of the University, because of a cheaper electricity contract there. This also rules the way data is stored for the simulations and work we do at HITS.

Data storage is structured across two (soon to be, three) high-speed filesystems: `fast` (directly located into the HITS facility, used for the HITS-located clusters), and `basement` (located at the same place as the *Genoa* cluster) to store and use the data generated on the cluster without having to transfer it elsewhere. 
...
To implement the methodological updates required for my project, for example enabling true 1D cartesian geometry mode or implementing a dynamic OpenMP parallelization, a dedicated branch was created from the main repository. Development workflows were designed with code merging in mind, allowing optimizations proven during the project to be integrated back into Phlegethon's main codebase. The 1D option will most likely be merged into the main as an option for more efficient computing for people needing no more than one dimension, and the OpenMP parallelization is worked upon by other people and alumnis of the PSO group to be more efficient and usable on GPU clusters instead of only CPU like it is now.
...
- *PSO Group Meeting (Wednesdays, 11:00 AM):* A joint session gathering the PSO team, members of the neighboring *SET* (Stellar Evolution Theory) group, and remote collaborators (including alumni now at institutions such as *Los Alamos National Laboratory*). Meetings combined operational updates (#gls("hpc") allocations, upcoming conferences) with a 40-to-60-minute scientific presentation. These presentations provided a platform for thesis defenses, conference dry-runs, and peer feedback across diverse topics in stellar physics. It showed the place that multi-disciplinary and diversification has for the group, which is one of the main benefits of HITS not being only a university department.
